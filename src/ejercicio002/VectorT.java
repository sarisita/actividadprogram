...
public class VectorT {
    ...
    public VectorT(JTextField jtam) {
        int i;
        try {

            tam = Integer.parseInt(jtam.getText());
            //Declarar el vector
            tienda = new TiendaE[tam];

            for (i = 0; i < tienda.length; i++) {
                tienda[i] = new TiendaE(0, "", "", 0.0, 0.0, 0);
            }
        } catch (Exception e) {
    ...
    public void setOrdenarPorNombre() {
        int n = tienda.length;
        for (int i = 1; i < n; i++) {
            TiendaE tien = tienda[i];
            int j = i - 1;
    ...
    public void setOrdenarPorStock() {
        int n = tienda.length;
        for (int i = 0; i < n - 1; i++) {
            int Max = i;
            for (int j = i + 1; j < n; j++) {
    ...
    public boolean setLlenarVec(
        ...
        try {
            if ((pos < 0) || (pos >= tam)) {
                ...
                tienda[pos].codigoProducto = Integer.parseInt(jtfCodP.getText());
                tienda[pos].nombreProducto = jtfnomP.getText();
                tienda[pos].marca = jtfmar.getText();
                tienda[pos].precioVenta = Double.parseDouble(jtfprecioV.getText());
                tienda[pos].precioCosto = Double.parseDouble(jtfprecioC.getText());
                tienda[pos].cantidadStock = Integer.parseInt(jtfcant.getText());
            }
            return true;
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null,
